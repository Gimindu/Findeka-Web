  ...
  const items = [
    {
      ...
      reward: 100,
      description:
        "Black iPhone 15 Pro with a blue case. Lost near the fountain.",
    ...
    },
    {
      ...
      reward: 0,
      description:
        "Brown leather wallet found on the street. Contains credit cards.",
      image: "/api/placeholder/200/200",
      user: "Jane Smith",
      matchScore: 85,
      status: "matched",
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            className={`px-6 py-3 ${colorClasses.accent} ${colorClasses.accentHover} rounded-lg font-medium transition-colors flex items-center gap-2`}
          >
            <Search size={20} />
            AI Match
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard
          title="Total Items"
          value={items.length}
          icon={<List className="text-orange-300" size={32} />}
        />
        <StatCard
          title="Matches Found"
          value={12}
        ...
        {filteredItems.map((item) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <h3
                className={`font-semibold text-lg mb-2 ${colorClasses.textPrimary}`}
              >
                {item.title}
              </h3>
              <p className={`text-sm ${colorClasses.textSecondary} mb-2`}>
                {item.category}
              </p>
              ...
              {item.reward > 0 && (
                <div
                  className={`text-sm font-medium ${colorClasses.textSecondary} mb-3`}
                >
                  Reward: ${item.reward}
                </div>
              )}
              ...
              <div className="flex gap-2">
                <button
                  className={`flex-1 py-2 px-3 ${colorClasses.primary} ${colorClasses.primaryHover} rounded-lg text-sm font-medium transition-colors`}
                >
                  Contact
                </button>
                <button
                  className={`px-3 py-2 border ${colorClasses.border} rounded-lg text-sm font-medium ${colorClasses.textSecondary} hover:bg-orange-50 transition-colors`}
                >
                  <Eye size={16} />
                </button>
              </div>
