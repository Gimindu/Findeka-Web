...
const ImageMatchingDemo = () => {
  ...
  const loadModel = async () => {
    ...
    { fromTFHub: true }
      ...
      setStatus('Model loaded! Add images to database, then search with query images.');
      setLoading(false);
    } catch (error) {
  ...
  const handleDatabaseUpload = async (event) => {
    ...
    setStatus(`Database updated! ${database.length + newDatabaseImages.length} images available for matching.`);
  };

  // Search with query image
  const handleQueryUpload = async (event) => {
    const file = event.target.files[0];
    if (!file || !model || database.length === 0) {
      setStatus('Please add images to database first, then upload a query image.');
      return;
    }
    ...
    img.onload = async () => {
      ...
      setStatus(`Found ${similarities.length} potential matches! Results sorted by similarity.`);
    };
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-4">
          Build a database of images, then upload a query image to find similar matches.
          This simulates a real-world image matching system.
        </p>
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          ...
          >
            {loading ? 'Loading Model...' : model ? 'Model Ready' : 'Load Model'}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
            📚 Image Database 
            <span className="text-sm font-normal text-gray-500">({database.length} images)</span>
          </h2>
          
          {database.length === 0 ? (
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center text-gray-500">
              No images in database yet.<br />
              <span className="text-sm">Upload multiple images to build your searchable database.</span>
            </div>
          ) : (
        ...
        <div>
          ...
          {!queryImage ? (
            <div className="border-2 border-dashed border-purple-300 rounded-lg p-8 text-center text-gray-500">
              No query image selected.<br />
              <span className="text-sm">Upload an image to search for similar matches in the database.</span>
            </div>
          ) : (
            <div className="border rounded-lg p-4">
              ...
              />
              <p className="text-sm font-medium text-center">{queryImage.name}</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="mt-8">
          ...
          <div className="space-y-3">
            {matches.map((match, index) => {
              ...
              return (
                <div key={match.image.id} className={`border rounded-lg p-4 ${
                  isHighMatch ? 'bg-green-50 border-green-200' : 
                  isMediumMatch ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50'
                }`}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">Added: {match.image.addedAt}</p>
                    </div>
                    
                    <div className="text-right">
                      <div className={`text-lg font-bold ${
                        isHighMatch ? 'text-green-600' : 
                        isMediumMatch ? 'text-yellow-600' : 'text-gray-600'
                      }`}>
                      ...
                      <div className="w-24 bg-gray-200 rounded-full h-2 mt-1">
                        <div
                          className={`h-2 rounded-full ${
                            isHighMatch ? 'bg-green-600' : 
                            isMediumMatch ? 'bg-yellow-600' : 'bg-gray-400'
                          }`}
                          style={{ width: `${similarity * 100}%` }}
                        ></div>
                      </div>
                      <div className="text-xs text-gray-500 mt-1">
                        {isHighMatch ? 'High Match' : isMediumMatch ? 'Medium' : 'Low Match'}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-lg">
        ...
        <ol className="text-sm text-gray-700 space-y-1 list-decimal list-inside">
          ...
          <li>Upload a query image using "Search Image" to find similar matches</li>
          <li>Results are ranked by similarity percentage (80%+ = high match)</li>
        </ol>
