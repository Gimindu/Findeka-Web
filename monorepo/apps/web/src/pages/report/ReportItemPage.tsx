      ...
      <div className="max-w-3xl mx-auto relative">
         ...
         <Card className="border-none shadow-sm">
            <CardHeader>
                ...
                <CardDescription>Please provide as much detail as possible to help AI match your item.</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    >
                        <div className="p-6 border-b border-slate-200">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="ghost" size="icon" onClick={() => setShowMatchesModal(false)}>
                                    <X className="h-6 w-6" />
                                </Button>
                            </div>
                            <p className="text-slate-500 mt-2">
                                Based on our AI analysis, these items look similar to what you reported. 
                                Please check if any of these are yours.
                            </p>
                        </div>

                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {matches.map((match) => (
                                <div key={match._id} className="border rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                                    ...
                                    <div className="p-4">
                                        ...
                                        <Button className="w-full" variant="outline" onClick={() => setSelectedMatch(match)}>
                                            View Details
                                        </Button>
                                    </div>
                                </div>
                        ...
                        <div className="p-6 bg-slate-50 border-t flex items-center justify-between">
                            ...
                            <Button onClick={handleNoMatch} className="bg-slate-900 text-white hover:bg-slate-800">
                                Submit My Report Anyway <ArrowRight className="ml-2 h-4 w-4" />
                            </Button>
                        </div>
                    </motion.div>
                </motion.div>
                ...
                >
                    ...
                    >
                         <div className="p-6 border-b border-slate-200 flex justify-between items-center">
                            <h2 className="text-xl font-bold text-slate-900">{selectedMatch.name}</h2>
                        ...
                        <div className="p-6 space-y-4">
                            {selectedMatch.image_url && (
                                <div className="h-64 w-full rounded-xl overflow-hidden bg-slate-100">
                                    <img src={selectedMatch.image_url} alt={selectedMatch.name} className="w-full h-full object-contain" />
                                </div>
                            )}
                            
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span className="font-semibold text-slate-700">Category:</span> {selectedMatch.category}
                                </div>
                                <div>
                                    <span className="font-semibold text-slate-700">Subcategory:</span> {selectedMatch.subcategory}
                                </div>
                                <div>
                                    <span className="font-semibold text-slate-700">Color:</span> {selectedMatch.color}
                                </div>
                                <div>
                                    <span className="font-semibold text-slate-700">Location:</span> {selectedMatch.location}
                                </div>
                                <div>
                                    <span className="font-semibold text-slate-700">Date:</span> {selectedMatch.date_found || selectedMatch.date_lost}
                                </div>
                                <div>
                                    <span className="font-semibold text-slate-700">Match Score:</span> {Math.round(selectedMatch.final_score * 100)}%
                                </div>
                            </div>
