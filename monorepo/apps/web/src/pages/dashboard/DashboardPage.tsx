    ...
    <DashboardLayout>
      ...
      <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-gradient-to-r from-[#DD6B20] to-[#FF8C42] rounded-2xl p-8 text-white relative overflow-hidden shadow-lg shadow-orange-500/20">
            <div className="relative z-10">
                ...
                <div className="flex gap-3">
                   <Link to="/report-item">
                    <Button variant="secondary" className="bg-white text-orange-600 hover:bg-orange-50 border-none">
        ...
        <div className="grid grid-rows-2 gap-4">
             <Card className="bg-white border-none shadow-sm flex flex-col justify-center relative overflow-hidden group hover:shadow-md transition-shadow">
                 <div className="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
                    <Trophy className="h-24 w-24 text-yellow-500" />
                 </div>
                 <CardContent className="p-6">
                    <div className="text-sm font-medium text-slate-500 mb-1">Your Impact</div>
                    <div className="text-3xl font-bold text-slate-800">120 pts</div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 md:pb-0">
             <Button 
                ...
                className={filterCategory === 'all' ? '' : 'bg-white border-none shadow-sm text-slate-600'}
             >
                All
             </Button>
             {categories.map(cat => (
                <Button 
                    ...
                    className={filterCategory === cat ? '' : 'bg-white border-none shadow-sm text-slate-600'}
                >
                    {cat}
                </Button>
             ))}
             <Button variant="outline" size="icon" className="bg-white border-none shadow-sm text-slate-600 shrink-0">
      ...
      >
        {filteredItems.map((item) => (
            <motion.div key={item.id} variants={itemAnim}>
                <Card className="overflow-hidden border-none shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 group bg-white h-full flex flex-col">
                    ...
                    <CardContent className="p-4 flex-1 flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                            <h3 className="font-bold text-lg text-slate-800 line-clamp-1 group-hover:text-[#DD6B20] transition-colors">{item.title}</h3>
                        </div>
