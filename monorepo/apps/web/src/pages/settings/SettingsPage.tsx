...
export default function SettingsPage() {
  ...
  const [profile, setProfile] = useState({ firstName: "", lastName: "", email: "", phone: "", about: "" });
  ...
  const handleSettingChange = (settingKey: string) => {
      setSettings(prev => ({ ...prev, [settingKey]: !(prev as any)[settingKey] }));
  };
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-6 pb-12">
            ...
            <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 md:col-span-2">
                        ...
                        <Input name="about" value={(profile as any).about || ""} onChange={handleProfileChange} />
                    </div>
                </div>
         ...
         <Card className="border-none shadow-sm">
            ...
            <CardContent className="space-y-4">
                {[
                    { key: "emailNotifications", title: "Email Notifications", desc: "Receive daily summaries and match alerts via email." },
                    { key: "pushNotifications", title: "Push Notifications", desc: "Get real-time alerts on your mobile device." },
                    { key: "matchAlerts", title: "Match Alerts", desc: "Get notified immediately when a potential match is found." },
                    { key: "newsletter", title: "Newsletter", desc: "Receive news and updates about Findeka features." },
                ].map((item) => (
         ...
         <Card className="border-none shadow-sm">
            <CardHeader>
             ...
             <CardContent className="space-y-4">
                 {[
                    { key: "publicProfile", title: "Public Profile", desc: "Allow other users to see your basic profile info." },
                    { key: "locationSharing", title: "Location Sharing", desc: "Share approximate location for better item matching." },
                ].map((item) => (
                    <div key={item.key} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                        <div className="space-y-0.5">
                            <h4 className="text-sm font-medium text-slate-900">{item.title}</h4>
